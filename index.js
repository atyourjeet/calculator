...
function calculate() {
  ...
  try {
    ...
    parsed = parsed.replace(/log\(/g, '__log10(');
    parsed = parsed.replace(/ln\(/g, '__log(');

    parsed = parsed.replace(/sinh\(/g, '__sinh(');
    parsed = parsed.replace(/cosh\(/g, '__cosh(');
    parsed = parsed.replace(/tanh\(/g, '__tanh(');

    parsed = parsed.replace(/(?<![a-zA-Z_])asin\(/g, '__asin(');
    parsed = parsed.replace(/(?<![a-zA-Z_])acos\(/g, '__acos(');
    parsed = parsed.replace(/(?<![a-zA-Z_])atan\(/g, '__atan(');

    parsed = parsed.replace(/(?<![a-zA-Z_])sin\(/g, '__sin(');
    parsed = parsed.replace(/(?<![a-zA-Z_])cos\(/g, '__cos(');
    parsed = parsed.replace(/(?<![a-zA-Z_])tan\(/g, '__tan(');
    ...
    do {
      prevParsed = parsed;
      parsed = parsed.replace(/(\d+(?:\.\d+)?|\)|__pi__|__E__|Ans|___SN_\d+___|!|%|²|³)\s*(\(|__pi__|__E__|Ans|__\w+)/g, '$1*$2');
      parsed = parsed.replace(/(\)|__pi__|__E__|Ans|___SN_\d+___|!|%|²|³)\s*(\d+(?:\.\d+)?|___SN_\d+___)/g, '$1*$2');
    ...
    parsed = parsed.replace(/__log10\(/g, 'Math.log10(');
    parsed = parsed.replace(/__log\(/g, 'Math.log(');
    ...
    const __yroot__ = (y, x) => {
      if (x < 0 && Math.abs(Math.round(y) % 2) === 1) {
    ...
    const evalFunc = new Function(
      'Ans', '_sin', '_cos', '_tan', '_asin', '_acos', '_atan', '__pct__', '__yroot__', '__fact__', '__perm__', '__comb__', '__smart_pow__',
      `return ${parsed};`
    );

    const result = evalFunc(lastAnswer, _sin, _cos, _tan, _asin, _acos, _atan, __pct__, __yroot__, __fact__, __perm__, __comb__, __smart_pow__);
...
document.addEventListener('keydown', (e) => {
  ...
  else if (key === 'l') { e.preventDefault(); appendFunc('log('); }
  else if (key === 'n') { e.preventDefault(); appendFunc('ln('); }
